...
for r in range(7):
    ...
    run(HANDLER(pl, e))
ev = FakeEvent(sender="727272", ts=T0 + 99999, text="help")
run(HANDLER(pl, ev))
check("T61 受阻陌生人帮助仍可查看", "普通数字" in ev.reply_text() and ev.stopped)
ev = FakeEvent(sender="727272", ts=T0 + 100000, text="123456789")
run(HANDLER(pl, ev))
check("T62 普通数字不会触发订单码兑换", "订单号" not in ev.reply_text() or "没有查到" not in ev.reply_text())
